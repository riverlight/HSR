...
def get_augment(img_list, hflip=True, rot=True):
...
def degradation_pipeline(img, degrade_seq=None):
    h, w, c = img.shape
    if degrade_seq is None:
...
def config_test():
    ...
    seq = config_to_seq(config)
    print_degrade_seg(seq)
    img = cv2.imread("d:/workroom/testroom/v0.png")
    img = degradation_pipeline(img, degrade_seq=seq)
    cv2.imwrite("d:/workroom/testroom/deg.png", img)
