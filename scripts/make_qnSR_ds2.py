...
train_lr_dir = "D:\\workroom\\tools\\dataset\\SR\\qnSR_DS\\train_lr"
train_h5 = "../qn_dataset/train_kg.h5"
...
def make_h5(dir_hr, dir_lr, h5name, interval=0, data_aug=True):
    h5_file = h5py.File(h5name, 'w')
    hr_patchs = list()
    lr_patchs = list()

    for count, name in enumerate(os.listdir(dir_hr)):
        imagename = os.path.join(dir_hr, name)
        lr_name = os.path.join(dir_lr, name.replace(".png", "_lr.png"))
        print(imagename, lr_name)
...
if __name__=="__main__":
    ...
    make_h5(train_dir, train_lr_dir, train_h5, interval=0, data_aug=True)
